...
const Step: FC<{ step: number;text: string;gradient: string }> = ({ step, text, gradient }) => (
	<Tabs.Trigger value={step.toString()} className="group flex gap-4 w-full text-left p-4 rounded-xl border transition-all duration-200 items-center border-neon-white/40 data-[state=active]:border-neon-white bg-neon-black bg-opacity-30">
...
const StepContent: FC<{ step: number; children: ReactNode }> = ({ step, children }) => (
	<Tabs.Content value={step.toString()} className="transition-all duration-500 px-6 py-4 rounded-xl border border-neon-white/10 text-neon-white/90 bg-neon-black bg-opacity-30">
		{children}
	</Tabs.Content>
);
...
const QuickStart: FC = () => (
	<section className="relative py-24 overflow-hidden">
		...
		<div className="max-w-6xl relative mx-auto px-4">
			<h2 className="text-4xl md:text-5xl font-bold text-center mb-4 text-neon-white">Quick Start</h2>
			...
			<Tabs.Root className="grid md:grid-cols-[300px,1fr] gap-8 min-h-[500px]" defaultValue="1">
				...
				<StepContent step={1}>
					<h3 className="text-2xl font-bold text-transparent text-neon-white bg-clip-text bg-gradient-to-r from-neon-purple to-neon-blue">Add to your flake inputs</h3>
					<p className="text-neon-white/80 text-lg">
						First, add NixCasks to your flake inputs to make the packages available.
					</p>
					<pre className="rounded-lg p-4">
						<code className="text-neon-white/90 text-sm">{INPUTS}</code>
					</pre>
				</StepContent>
				<StepContent step={2}>
					<h3 className="text-2xl font-bold text-transparent text-neon-white bg-clip-text bg-gradient-to-r from-neon-blue to-neon-green">Install packages</h3>
					<p className="text-neon-white/80 text-lg">
					...
					<div className="space-y-6 mt-6">
						<div className="space-y-2">
							...
							<pre className="rounded-lg p-4">
								<code className="text-neon-white/90 text-sm">{SYSTEM}</code>
							</pre>
						</div>
						<div className="space-y-2">
							<h4 className="font-bold text-neon-white/90">Per-user installation (home-manager):</h4>
							<pre className="rounded-lg p-4">
								<code className="text-neon-white/90 text-sm">{HOME_MANAGER}</code>
							</pre>
						</div>
					</div>
				</StepContent>
				<StepContent step={3}>
					<h3 className="text-2xl font-bold text-transparent text-neon-white bg-clip-text bg-gradient-to-r from-neon-green to-neon-yellow">Rebuild your system</h3>
					<p className="text-neon-white/80 text-lg">
						Rebuild your system to apply the changes:
					</p>
					<pre className="rounded-lg p-4">
						<code className="text-neon-white/90 text-sm">{REBUILD}</code>
					</pre>
				</StepContent>
			</Tabs.Root>
